...
class pgFDW_mapd(ForeignDataWrapper):
    ...
    def __init__(self, options,columns):
        ...
        for attribute in connection_attrs.keys():
            if attribute not in options:
                log_to_postgres('The '+attribute+' parameter should be defined. defaulting to :'+connection_attrs.get(attribute),WARNING)
                setattr(self, attribute, connection_attrs.get(attribute))
            else:
                setattr(self, attribute, options.get(attribute))
        self.connection = connect(user = self.user, password = self.password, host = self.host, port = self.port, dbname = self.dbname)
        self.idle_timeout = options.get('idle_timeout',300)
        self.idle_timeout_timer = Timer(self.idle_timeout, self.close_connection)
    ...
    def close_connection(self):
        log_to_postgres("Idle timeout occurred; the Session has been disconnected",INFO)
    ...
    def execute(self, quals, columns):
        log_to_postgres("timer ."+str(self.idle_timeout_timer.is_alive),INFO)
        if self.connection == None or self.connection.closed == 1:
            self.connection = connect(user = self.user, password = self.password, host = self.host, port = self.port, dbname = self.dbname)
        ...
        statement = u""
        statement = "SELECT {0} from {1}".format(",".join(columns),self.table_name)
        isWhereDefined = False
        for qualifier in quals:
            translated_qo = translate_qual_operator.get(qualifier.operator,qualifier.operator)
            if translated_qo == 'not ilike':
                log_to_postgres('The operator '+translated_qo+' is not supported on MAPD',ERROR)
                self.connection.close()
                return
            if isWhereDefined == False:
            ...
            if translated_qo == qualifier.operator or translated_qo != 'in':
                statement = statement + " " + self.return_formatted_value(qualifier.value)
            elif translated_qo == 'in':
                statement = statement + " ( {0} )".format(",".join([ self.return_formatted_value(in_value) for in_value in qualifier.value]))
            if limit != -1:
